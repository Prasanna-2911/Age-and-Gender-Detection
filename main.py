...
if uploaded_file is not None:
    ...
    face_txt_path="opencv_face_detector.pbtxt.txt"
    face_model_path="opencv_face_detector_uint8.pb"

    age_txt_path="age_deploy.prototxt.txt"
    age_model_path="age_net.caffemodel"

    gender_txt_path="gender_deploy.prototxt.txt"
    ...
    for bbox in b_boxes:
        ...
        st.write(f"{gender}, confidence = {gender_pred_list[0].max() * 100}%")

        age_net.setInput(blob)
        age_pred_list = age_net.forward()
        age = age_classes[age_pred_list[0].argmax()]
        st.write(f"{age}, confidence = {age_pred_list[0].max() * 100}%")
